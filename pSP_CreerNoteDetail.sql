...
/****** Object:  StoredProcedure [dbo].[pSP_CreerNoteDetail]    Script Date: 3/5/2026 10:35:48 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
                ...
                BEGIN TRANSACTION;
                              ...
                              ;WITH CTE_VALEUR_GLOBALE  AS
                               ...
                               CTE_AJUSTEMENT_GLOBAL AS
                                (
                                               SELECT @ValeurTotaleColisage - SUM([Valeur Globale]) AS [Ajustement Global]
                                               FROM CTE_VALEUR_GLOBALE
                               ),
                               CTE_LIGNE_MAX AS
                                (
                                               SELECT TOP 1 [Colisage Dossier]
                                               FROM CTE_VALEUR_DC
                                               ORDER BY [Valeur Globale] DESC
                               )
                               INSERT INTO [dbo].[TNotesDetail]
                                                                               ([Colisage Dossier]
                                                                               ,[Regime]
                               ...
                               -- Cas TR avec ajustement sur la ligne ayant la plus grande valeur
                               UNION ALL
                                
                               SELECT A.[ID Colisage Dossier]
                                               ,FORMAT(1-B.[Taux Regime],'P')  + N'TR'
                                               ,B.[Valeur Globale] - B.[Valeur DC] + CASE WHEN EXISTS(SELECT 1 FROM CTE_LIGNE_MAX M WHERE M.[Colisage Dossier]=B.[Colisage Dossier]) THEN (SELECT [Ajustement Global] FROM CTE_AJUSTEMENT_GLOBAL) ELSE 0 END
                               ...
                               -- AJUSTEMENT DES VALEURS DE COLISAGE SUR LES TOTAUX (POIDS ET VOLUMES UNIQUEMENT)
